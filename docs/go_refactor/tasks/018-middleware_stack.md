...
- [x] `backend/internal/middleware/auth.go` — validates session token from cookie/header, attaches
  archer ID to request context, returns 401 if invalid.
- [x] `backend/internal/middleware/error_mapper.go` — catches `apperror.*` types from handlers and
  maps them to HTTP status codes:
    ...
    - `ErrValidation` → 422
    - Unknown errors → 500
- [x] `backend/internal/middleware/logging.go` — logs method, path, status, duration for each
  request using `slog`.
- [x] `backend/internal/middleware/cors.go` — configures CORS headers based on dev/prod mode.
- [x] `backend/internal/middleware/recovery.go` — recovers from panics and returns 500.
- [x] `backend/internal/middleware/context.go` — helpers to get/set archer ID in request context.
- [x] Unit tests cover:
...
- [x] **Step 2: Write failing tests for auth middleware**
...
- [x] **Step 3: Write failing tests for context helpers**
...
- [x] **Step 5: Implement all middleware files**
